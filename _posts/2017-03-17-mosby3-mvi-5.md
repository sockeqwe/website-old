...
tags: [android, java]
---
In the previous blog posts we have talked about Model-View-Intent (MVI) and it's characteristics. In this blog post we will discuss how MVI simplifies the life of us, the developers.
...
Now we will see how this pays off when it comes to debugging to simplify the life of developers.

Have you ever get a crash report and were not able to reproduce the bug? Sounds familiar?
Well, for me too. After having spend many hours looking at the stacktrace and our source code many times I gave up and have closed such issues in our issue tracker with a little comment like "can't reproduce it" or "must be a strange device / manufacturer specific bug".
...
as the crash reports states, there is a NullPointerException be thrown while loading the newest data triggered by that pull-to-refresh:
...
class HomePresenter extends MviBasePresenter<HomeView, HomeViewState> {
  ...
  @Override protected void bindIntents() {
    ...
    Observable<HomeViewState> stateObservable = allIntents
          ...
          .doOnNext(newViewState -> Crashlytics.log( gson.toJson(newViewState) ));
...
Please ping him if you want to offer some help.
