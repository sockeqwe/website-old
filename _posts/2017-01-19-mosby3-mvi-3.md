...
The UI element that indicates that more items of that category can be loaded is a FeedItem:
...
Now let's connect the dots step by step in our Presenter. Please note that some code shown here as part of the Presenter should rather be moved into a Interactor in a real world application (which I didn't for the sake of better readability).
