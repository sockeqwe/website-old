...
If you compare both, the original repository pattern and the later one, you will see that actually they are trying to solve two different kind of problems. While the original one tries to minimizing duplicate query logic by the open / closed principle the later one tries to hide the concrete data store your app is talking to. But you can't really save both problems at the same time within the same pattern as Krzychu Kosobudzki tried in his blog post. You have to choose which problem you want to solve in your app (although you can chain both patterns together).
