...
Last but not least I want to say that my intention of this blog post was not to discredit SoundCloud's LigthCycle. I just wanted to say that from my point of view the LightCycle [examples](https://github.com/soundcloud/lightcycle/blob/master/examples/real-world/src/main/java/com/soundcloud/lightcycle/sample/real_world/HeaderPresenter.java) (having Presenters with lifecycle callback methods) are not my cup of tea. I think this is also caused by the fact that my definition of a Presenter (i.e. I also don't want to have Android SDK dependencies like Bundle in my Presenter) is entirely different from SoundClouds definition of a Presenter.
