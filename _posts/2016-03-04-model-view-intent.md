...
See the thig with MVI is like with any other software architecture: MVI gives you an idea, but there is enough space for personal preferences and interpretation. One can add as many layers as needed. For example, the `model()` function could internally be composed by multiple smaller `method()` functions (one might call them use cases or interactors, just functional). One could add `Action` data structures to decouple things between `intent()` and `model()` even more. But keep in mind: **don't over-engineering things!** Software architecture is a continuous evolution:
