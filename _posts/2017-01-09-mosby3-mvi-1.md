...
This leads to the following observations:
 ...
 2. A Presenter (or ViewModel) has arbitrary many inputs (the View triggers an action handled by Presenter) which is ok, but Presenter has many outputs (or output channels like view.showLoading() or view.showError() in MVP or ViewModel is offering multiple Observables) which eventually leads to conflicting states of View, Presenter and business logic especially when working with multiple threads.
...
I'm pretty sure that there are also other solutions for View's state problem out there. Let's take a step back and summarize the issue those libraries try to solve: They try to solve the problem of state which we have already discussed.
...
![Counter](/images/mvi-mosby3/counter.png)
